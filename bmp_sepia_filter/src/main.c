...
void pre(void *_pre_args);
void pre_loop(void *_pre_args);
void post_loop(void *_pre_args);
...
int main()
{
    struct bmp_header header;
    struct image image;
    struct args args = {IMAGE_NAME, "general", NULL, &header, &image};
    void *_args = &args;

    bench_a("general", COUNT, apply_sepia_filter, _args, pre, _args, NULL, NULL, pre_loop, _args, post_loop, _args);
    args.label = "avx";
    bench_a("avx", COUNT, apply_sepia_filter_avx, _args, pre, _args, NULL, NULL, pre_loop, _args, post_loop, _args);
    args.label = "haxe";
    bench_a("haxe", COUNT, apply_sepia_filter_haxe, _args, pre, _args, NULL, NULL, pre_loop, _args, post_loop, _args);
...
void pre(void *_pre_args)
{
    struct args *args = _pre_args;
    const char *label = args->label;

    args->image_name_o = malloc(sizeof(char) * 30);
    sprintf(args->image_name_o, "%s.bmp", label);
}

void pre_loop(void *_pre_loop_args)
{
    struct args *args = _pre_loop_args;
    const char *image_name = args->image_name;
    struct bmp_header *header = args->header;
    struct image *image = args->image;

    load_bmp(image_name, header, image);
}

void post_loop(void *_post_loop_args)
{
    struct args *args = _post_loop_args;
    const char *image_name_o = args->image_name_o;
    struct bmp_header *header = args->header;
    struct image *image = args->image;
...
{
    struct args *args = _post_args;
    
    free(args->image_name_o);
}
...
static void mul_matrix_sepia(struct pixel *const p)
{
    ...
    uint8_t *vp = (void *)p;
    __m128 xm_b, xm_g, xm_r, bgr1, bgr2, bgr3;
    __m128i inprod;
